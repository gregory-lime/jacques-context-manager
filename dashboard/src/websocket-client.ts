...
export class JacquesClient extends EventEmitter {
  ...
  connect(): void {
    ...
    try {
      ...
      this.ws.on('open', () => {
        ...
        console.log('[Client] Connected to Jacques server');
        this.emit('connected');
      });
      ...
      this.ws.on('close', () => {
        ...
        this.ws = null;
        console.log('[Client] Disconnected from Jacques server');
  ...
  private handleMessage(data: Buffer): void {
    try {
      ...
      switch (message.type) {
        ...
        default:
          console.warn(`[Client] Unknown message type: ${(message as ServerMessage).type}`);
      }
    } catch (err) {
  ...
  private scheduleReconnect(): void {
    ...
    console.log(`[Client] Reconnecting in ${this.reconnectDelay / 1000}s...`);
